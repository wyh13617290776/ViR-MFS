...
def train():
    ...
    if os.path.exists(checkpoint_path):
        ...
        # 加载到模型中
        # strict=True 表示必须完全匹配，这能帮你检查模型结构是否改动过
        model.load_state_dict(new_state_dict, strict=True)
        logger.info(f"成功从 {checkpoint_path} 加载完整模型权重，准备开始续训/微调。")
    else:
        logger.error(f"未找到最佳权重文件: {checkpoint_path}，请检查路径！")
    # --- 修改后 ---
    ...
    for epoch in range(t_params['epochs']):
        ...
        for vi, ir, lbl in pbar:
            ...
            if torch.isnan(L_total) or torch.isinf(L_total):
                continue  # 一旦 Loss 爆炸，直接抛弃这个 Batch！
            # ====================================================
            scaler.scale(L_total).backward()
            # ================= [新增梯度裁剪安全阀] =================
            scaler.unscale_(opt_all) # 先把 FP16 的 scale 缩放还原
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0) # 强制砍掉超过 5.0 的异常梯度
            # =======================================================
            scaler.step(opt_all)
            scaler.update()
            ...
            if (epoch >= t_params['inner_warmup']) and ((epoch % t_params['inner_every']) == 0) and (len(F_params) > 0) and (epoch % 2 == 0):
                ...
                if vi_mtr.shape[0] > 0 and vi_mts.shape[0] > 0:
                    ...
                    # ========== [核级护盾 1：拦截一阶毒梯度] ==========
                    if any(torch.isnan(g).any() or torch.isinf(g).any() for g in grads_F):
                        continue  # 发现毒梯度，直接放弃当前 Batch 保护模型！
                    # ==================================================
                    ...
                    if opt_F is not None:
                        ...
                        # ========== [核级护盾 2：拦截二阶毒梯度] ==========
                        has_inf_nan = any(torch.isnan(p.grad).any() or torch.isinf(p.grad).any() for p in F_params if p.grad is not None)
                        if not has_inf_nan:
                            torch.nn.utils.clip_grad_norm_(F_params, max_norm=5.0)
                            opt_F.step()
                        # 如果发现 NaN，这里会什么都不做，安全跳过本次更新
                        # ==================================================
            ...
            # ========== [新增：全局前向传播 NaN 嗅探器] ==========
            if torch.isnan(L_total2) or torch.isinf(L_total2):
                continue  # 一旦 Loss 爆炸，直接抛弃这个 Batch！
            # ====================================================
            ...
            if (epoch >= t_params['inner_warmup']) and ((epoch % t_params['inner_every']) == 0) and (len(S_params) > 0) and (epoch % 2 != 0):
                ...
                if (lbl_mtr is not None) and (vi_mtr.shape[0] > 0) and (vi_mts.shape[0] > 0):
                    ...
                    # ========== [核级护盾 3：拦截一阶毒梯度] ==========
                    if any(torch.isnan(g).any() or torch.isinf(g).any() for g in grads_S):
                        continue  # 发现毒梯度，直接放弃当前 Batch 保护模型！
                    # ==================================================
                    ...
                    if opt_S is not None:
                        ...
                        # ========== [核级护盾 4：拦截二阶毒梯度] ==========
                        has_inf_nan = any(torch.isnan(p.grad).any() or torch.isinf(p.grad).any() for p in S_params if p.grad is not None)
                        if not has_inf_nan:
                            torch.nn.utils.clip_grad_norm_(S_params, max_norm=5.0)
                            opt_S.step()
                        # ==================================================
